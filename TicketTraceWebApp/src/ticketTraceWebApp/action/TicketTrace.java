 ...
 * 
 */
package ticketTraceWebApp.action;
...
{
  static Logger logger = Logger.getLogger(TicketTrace.class.getName());

  public ActionForward unspecified(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
    throws Exception
  {
    logger.info(" - Action - TicketTrace - unspecified ::: ");
  ...
  {
    logger.info(" - Action - getTicketTraceReport - unspecified ::: ");
    ...
    try
    {
      ...
      ticketFrom = request.getParameter("ticketFrom");
      ticketTo = request.getParameter("ticketTo");
      ...
      if ((ticketNo != null) && (ticketNo != "")) {
        intChkRadio = 0;
        ticketTraceResultsForm.setTicketno(ticketNo);
      } else if ((ticketFrom != null) && (ticketFrom != "") && (ticketTo != null) && (ticketTo != ""))
      {
        intChkRadio = 1;
        ticketTraceResultsForm.setTicketFrom(ticketFrom);
        ticketTraceResultsForm.setTicketTo(ticketTo);
      } else if ((fromDate != null) && (fromDate != "") && (toDate != null) && (toDate != ""))
      {
        intChkRadio = 2;
        ticketTraceResultsForm.setStartDate(fromDate);
        ticketTraceResultsForm.setEndDate(toDate);
      }
      logger.info("intChkRadio is" + intChkRadio);
      session.setAttribute("RADIO", Integer.valueOf(intChkRadio));

      logger.info("ticketno : " + ticketNo + "ticketFrom : " + ticketFrom + "ticketTo : " + ticketTo + "fromDate :" + fromDate + "toDate :" + toDate);
      ...
      logger.info("Using OracleCallableStatement / OracleResultSet");
      ...
      while (deptResultSet.next()) {
        ...
        while (deptResultSetActivity.next()) {
          ...
          String strActivityLoggedDate = activityVO.getActivityLoggedDate();

          if (strActivityLoggedDate != null) {
            Date date2 = sdfSource.parse(strActivityLoggedDate);
            strActivityLoggedDate = sdfDestination.format(date2);
            activityVO.setActivityLoggedDate(strActivityLoggedDate);
          }
      ...
      oraCallStmt.close();
      conn.close();
      logger.info("SIZE OF RESULT LIST IS :_" + ticketTraceResultsForm.getTicketTraceResultList().size());
  ...
  public ActionForward generateexportpdf(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
    throws Exception
  {
    try
    {
      logger.info("in generateexportpdf >>>>>>>");
      ...
      logger.info("ticketTraceReport size is >>>>>>>>" + ticketTraceReport.size());

      String DATE_FORMAT_NOW = "MM/dd/yyyy HH:mm:ss";
      Calendar cal = Calendar.getInstance();
      SimpleDateFormat sdf = new SimpleDateFormat(DATE_FORMAT_NOW);
      String systemTime = sdf.format(cal.getTime());
      systemTime = systemTime.replaceAll("/", "_");
      systemTime = systemTime.replaceAll(":", "_");

      response.setContentType("application/pdf");
      response.setHeader("Content-disposition", "attachment ;filename=TicketTrace" + systemTime + ".pdf");
  ...
  public ActionForward generateexportWord(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
    throws Exception
  {
    try
    {
      ...
      response.setHeader("Content-disposition", "attachment ;filename=TicketTrace_" + systemTime + ".doc");
