...
/**
 * Servlet implementation class ResolvedByHoController
 */
@WebServlet("/ResolvedByHoController")
public class ResolvedByHoController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ResolvedByHoController() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(action.equals("ResolvedByHoList")){
			request.setAttribute("controllerName", "ResolvedByHoController");
		...
		Service service =new Service();
			...
			RequestDispatcher rd = request.getRequestDispatcher("jsp/ResolvedByHo.jsp");
			logger.info("InfoMessage:data successfullly return from dao and send to jsp of ResolvedByHo  Status:success");
			rd.forward(request, response);}
			else {
				logger.error("ErroMessage:data recived by Reopen10To3pmDao is either empty or null  Status:failure " +"Exception:please check controller and dao of Reopen10To3pmDao");
				RequestDispatcher rd = request.getRequestDispatcher("jsp/errorPage.jsp");
				rd.forward(request, response);	
				}
		}catch(Exception e){
			logger.error("ErroMessage:request not forwarded to ResolvedByHo jsp  Status:failure " +"Exception:" +e.getStackTrace());
			RequestDispatcher rd = request.getRequestDispatcher("jsp/errorPage.jsp");
			rd.forward(request, response);
			}
	}
	else {
		logger.error("ErroMessage:action not received from ResolvedByHo jsp  Status:failure " +"Exception:please check ResolvedByHO controller and dao" );
		RequestDispatcher rd = request.getRequestDispatcher("jsp/errorPage.jsp");
		rd.forward(request, response);	
		}
	}
